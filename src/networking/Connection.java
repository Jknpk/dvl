...
public class Connection {
	...
	public MoveMessageType awaitMove(HashMap<Integer, Player> spieler, Board brett, int tries,
		...
		if (spieler.get(player.getID()) != null && tries < Settings.MOVETRIES) {
			sendMessage(mazeComMessageFactory.createAwaitMoveMessage(spieler, player.getID(), brett, foundTreasures),
					true);
	...
	@Deprecated
	public MoveMessageType awaitMove3(HashMap<Integer, Player> spieler, Board brett, int tries,
			List<TreasureType> foundTreasures) {
		this.sendMessage(
				this.mazeComMessageFactory.createAwaitMoveMessage(spieler, this.player.getID(), brett, foundTreasures),
				true);
		MazeCom result = this.receiveMessage();
		if (result != null && result.getMcType() == MazeComType.MOVE) {
			// Antwort mit NOERROR
			if (this.currentGame.getBoard().validateTransition(result.getMoveMessage(), this.player.getID())) {
				this.sendMessage(this.mazeComMessageFactory.createAcceptMessage(this.player.getID(), ErrorType.NOERROR),
						false);
				return result.getMoveMessage();
		...
		this.sendMessage(this.mazeComMessageFactory.createAcceptMessage(this.player.getID(), ErrorType.AWAIT_MOVE),
				false);
	...
	public MoveMessageType illegalMove(HashMap<Integer, Player> spieler, Board brett, int tries,
			List<TreasureType> foundTreasures) {
		this.sendMessage(this.mazeComMessageFactory.createAcceptMessage(this.player.getID(), ErrorType.ILLEGAL_MOVE),
				false);
		if (tries < Settings.MOVETRIES)
	...
	public void sendWin(int winnerId, String name, Board b) {
		this.sendMessage(this.mazeComMessageFactory.createWinMessage(this.player.getID(), winnerId, name, b), false);
		try {
	...
	public void disconnect(ErrorType et) {
		this.sendMessage(
				this.mazeComMessageFactory.createDisconnectMessage(this.player.getID(), this.player.getName(), et),
				false);
		try {
			this.inFromClient.close();
			this.outToClient.close();
