	...
	public void start() {
		...
		while (true) {
			...
			if (mc.getMcType() == MazeComType.AWAITMOVE) {
				...
				MazeCom mcMM = mazeComMessageFactory.createMoveMessage(client.getId(),
						taktik.getMove(mc.getAwaitMoveMessage(), id));
				...
				if (mcAnswer.getMcType() == MazeComType.ACCEPT) {
					if (mcAnswer.getAcceptMessage().isAccept()) {
						...
						System.err.println("Falsche Nachricht!");
							...
							&& mcAnswer.getAcceptMessage().getErrorCode() == ErrorType.ILLEGAL_MOVE) {
